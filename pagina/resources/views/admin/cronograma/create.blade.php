...
<style>
    ...
    .btn-save {
        ...
        padding: 0.7rem 1.5rem; font-family: 'Sora', sans-serif; font-size: 0.9rem; font-weight: 600; cursor: pointer;
    }
...
<div class="form-card">
    <div class="form-title">Agregar nuevo slot al cronograma</div>
    @if($errors->any())
    ...
    @endif
    <form method="POST" action="{{ route('admin.cronograma.store') }}">
        @csrf
        <div class="field-row">
            <div class="field-group">
                <label>Día (etiqueta)</label>
                <input type="text" name="day_label" value="{{ old('day_label') }}" placeholder="Jueves 23 Jul"/>
                @error('day_label')<div class="field-error">{{ $message }}</div>@enderror
            </div>
            <div class="field-group">
                <label>Clave del día</label>
                <select name="day_key">
                    <option value="d1" {{ old('day_key') == 'd1' ? 'selected' : '' }}>d1 — Día 1</option>
                    <option value="d2" {{ old('day_key') == 'd2' ? 'selected' : '' }}>d2 — Día 2</option>
                    <option value="d3" {{ old('day_key') == 'd3' ? 'selected' : '' }}>d3 — Día 3</option>
                </select>
            </div>
        </div>
        <div class="field-group">
            <label>Etiqueta del día</label>
            <input type="text" name="day_tag" value="{{ old('day_tag') }}" placeholder="Simposio Día 1"/>
            @error('day_tag')<div class="field-error">{{ $message }}</div>@enderror
        </div>
        <div class="field-row">
            <div class="field-group">
                <label>Hora inicio</label>
                <input type="text" name="time_start" value="{{ old('time_start') }}" placeholder="09:00"/>
                @error('time_start')<div class="field-error">{{ $message }}</div>@enderror
            </div>
            <div class="field-group">
                <label>Hora fin (opcional)</label>
                <input type="text" name="time_end" value="{{ old('time_end') }}" placeholder="12:30"/>
            </div>
        </div>
        <div class="field-group">
            <label>Título del slot</label>
            <input type="text" name="title" value="{{ old('title') }}" placeholder="Módulo I: Enfermedad Cerebrovascular"/>
        ...
        <div class="field-row">
            ...
            <div class="field-group">
                <label>&nbsp;</label>
                <div class="check-row">
                    <input type="checkbox" id="is_break" name="is_break" value="1" {{ old('is_break') ? 'checked' : '' }}/>
                    <label for="is_break" style="margin:0; font-weight:400;">¿Es descanso/almuerzo?</label>
                </div>
            </div>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn-save">Guardar slot</button>
