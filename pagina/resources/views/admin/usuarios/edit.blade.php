...
.info-box { background:#fdf9ec; border:1px solid #fde68a; border-radius:.5rem; padding:.75rem 1rem; font-size:.84rem; color:#92400e; margin-bottom:1.25rem; }
...
<div class="card">
    <div class="card-title">Editar usuario: <code style="font-size:.9rem;">{{ $usuario->nickname }}</code></div>

    @if($usuario->registro)
        <div class="info-box">
            Vinculado al participante: <strong>{{ $usuario->registro->nombre_completo }}</strong>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.usuarios.update', $usuario) }}">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input class="form-input" name="name" value="{{ old('name', $usuario->name) }}" required>
                @error('name')<div class="error-msg">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">Nickname *</label>
                <input class="form-input" name="nickname" value="{{ old('nickname', $usuario->nickname) }}" required>
                @error('nickname')<div class="error-msg">{{ $message }}</div>@enderror
            </div>
        </div>
        <div class="form-group">
            <label class="form-label">Correo electrónico</label>
            <input class="form-input" type="email" name="email" value="{{ old('email', $usuario->email) }}">
        </div>
        <div class="form-row">
            <div class="form-group">
                <label class="form-label">Nueva contraseña</label>
                <input class="form-input" type="password" name="password" placeholder="Dejar en blanco para no cambiar">
        ...
        <div class="form-group">
            ...
            <select class="form-select" name="role" required {{ $usuario->id === auth()->id() ? 'disabled' : '' }}>
                <option value="participante" {{ $usuario->hasRole('participante') ? 'selected' : '' }}>Participante</option>
                <option value="admin" {{ $usuario->hasRole('admin') ? 'selected' : '' }}>Administrador</option>
            </select>
            @if($usuario->id === auth()->id())
                <input type="hidden" name="role" value="{{ $usuario->roles->first()?->name }}">
