...
<div class="form-card">
    <div class="form-title">Editar precio</div>
    @if($errors->any())
    ...
    @endif
    <form method="POST" action="{{ route('admin.precios.update', $precio) }}">
        @csrf
        @method('PUT')
        <div class="field-group">
            <label>Evento</label>
            <select name="event">
                <option value="simposio" {{ old('event', $precio->event) == 'simposio' ? 'selected' : '' }}>Simposio</option>
                <option value="taller" {{ old('event', $precio->event) == 'taller' ? 'selected' : '' }}>Taller</option>
            </select>
        </div>
        <div class="field-group">
            <label>Categoría profesional</label>
            <input type="text" name="category" value="{{ old('category', $precio->category) }}"/>
            @error('category')<div class="field-error">{{ $message }}</div>@enderror
        </div>
        <div class="field-group">
            <label>Precio (Bs.)</label>
            <input type="number" name="price" value="{{ old('price', $precio->price) }}" min="0" step="0.01"/>
            @error('price')<div class="field-error">{{ $message }}</div>@enderror
        </div>
        <div class="field-group">
            <label>Nota (opcional)</label>
            <input type="text" name="note" value="{{ old('note', $precio->note) }}"/>
        </div>
        <div class="field-group">
            <label>Orden</label>
            <input type="number" name="sort_order" value="{{ old('sort_order', $precio->sort_order) }}" min="0"/>
        </div>
